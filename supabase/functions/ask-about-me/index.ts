...
/**
 * Chamada real à API da OpenAI (gpt-4o-mini).
 * A chave nunca é hardcoded nem enviada ao navegador — vem só do secret
 * configurado no Supabase (Deno.env.get).
 *
 * Se a chamada falhar por qualquer motivo (chave não configurada, billing
 * pendente na OpenAI, API fora do ar, limite de uso), cai automaticamente
 * no modo de respostas prontas (fallbackAnswer), baseado na sua própria
 * base de conhecimento — sem gerar erro para quem está usando o chat.
 */
